...
public class FhirR4 {
  ...
  protected static boolean USE_US_CORE_IG =
      Config.getAsBoolean("exporter.fhir.use_us_core_ig", true);
  protected static boolean USE_FR_CORE_IG =
  ...
  private static BundleEntryComponent basicInfo(Person person, Bundle bundle, long stopTime) {
    ...
    //TODO Crash if neither FR or US Core ?
    if (USE_US_CORE_IG) {
      patientResource.getMeta().addProfile("http://hl7.org/fhir/us/core/StructureDefinition/us-core-patient");
      ...
      //Address
      String state = Location.getAbbreviation((String) person.attributes.get(Person.STATE));
      Address address = patientResource.addAddress();
      address.addLine((String) person.attributes.get(Person.ADDRESS))
      ...
      // Standard Health Records Extensions
    ...
    List<PatientCommunicationComponent> communication = new ArrayList<>();
...
//    }
  ...
  private static BundleEntryComponent encounter(Person person, BundleEntryComponent personEntry,
    ...
    if (USE_US_CORE_IG) {
      encounterResource.getMeta().addProfile("http://hl7.org/fhir/us/core/StructureDefinition/us-core-encounter");
      ...
      encounterResource.getMeta().addProfile("https://hl7.fr/ig/fhir/core/StructureDefinition/fr-core-encounter");
      encounterResource.addIdentifier()
              .setUse(IdentifierUse.OFFICIAL)
  ...
  /**
   * Add an entry for Appointment (FR Core Only)
   *
  ...
  private static BundleEntryComponent appointment(BundleEntryComponent personEntry, Bundle bundle, Encounter encounter) {
    ...
    appointmentResource.setStatus(Appointment.AppointmentStatus.FULFILLED);

    // Clinician
    if (encounter.clinician != null) {
      if (TRANSACTION_BUNDLE) {
        appointmentResource.addParticipant().setActor(new Reference(
                ExportHelper.buildFhirNpiSearchUrl(encounter.clinician)).setDisplay(encounter.clinician.getFullname()))
                .setStatus(Appointment.ParticipationStatus.ACCEPTED);
      } else {
        String practitionerFullUrl = findPractitioner(encounter.clinician, bundle);
        if (practitionerFullUrl != null) {
          appointmentResource.addParticipant().setActor(new Reference(practitionerFullUrl).setDisplay(encounter.clinician.getFullname()))
                  .setStatus(Appointment.ParticipationStatus.ACCEPTED);
        } else {
          BundleEntryComponent practitioner = practitioner(bundle, encounter.clinician);
          appointmentResource.addParticipant().setActor(new Reference(practitioner.getFullUrl()).setDisplay(encounter.clinician.getFullname()))
                  .setStatus(Appointment.ParticipationStatus.ACCEPTED);
        }
      }
    }
  ...
  private static BundleEntryComponent observation(
    ...
    if (USE_US_CORE_IG) {
      Meta meta = observationResource.getMeta();
  ...
  protected static BundleEntryComponent provider(Bundle bundle,
    ...
    if (USE_US_CORE_IG) {
      organizationResource.getMeta().addProfile("http://hl7.org/fhir/us/core/StructureDefinition/us-core-organization");

      //Type
      List<CodeableConcept> organizationType = new ArrayList<>();
    ...
    if (USE_FR_CORE_IG) {
      organizationResource.getMeta().addProfile("https://hl7.fr/ig/fhir/core/StructureDefinition/fr-core-organization");
  ...
  protected static BundleEntryComponent practitioner(Bundle bundle,
    ...
    if (USE_FR_CORE_IG) {
      ...
      // Name
      HumanName practitionerName = practitionerResource.addName().setFamily(
                      (String) clinician.attributes.get(Clinician.LAST_NAME))
              .addGiven((String) clinician.attributes.get(Clinician.FIRST_NAME));
      String prefix = (String) clinician.attributes.get(Clinician.NAME_PREFIX);
      if ("Dr.".equals(prefix)) {
        practitionerName.addSuffix("DR");
      }
    }
    ...
    // Email
    String email = clinician.attributes.get(Clinician.FIRST_NAME)
        + "." + clinician.attributes.get(Clinician.LAST_NAME)
        + "@example.com";
    ...
    if (USE_FR_CORE_IG) {
      // generate accompanying PractitionerRoles resource

      // Exercice
      PractitionerRole practitionerRoleExercice = new PractitionerRole();
      // Profile
      practitionerRoleExercice.getMeta().addProfile("https://hl7.fr/ig/fhir/core/StructureDefinition/fr-core-practitioner-role-exercice");
      // Practitioner
      practitionerRoleExercice.setPractitioner(new Reference()
              .setIdentifier(new Identifier()
                      .setSystem("http://rpps.esante.gouv.fr")
                      .setValue(clinician.rpps))
              .setDisplay(practitionerResource.getNameFirstRep().getNameAsSingleString()));
      // Organization
      practitionerRoleExercice.setOrganization(new Reference()
              .setIdentifier(new Identifier()
                      .setSystem(SYNTHEA_IDENTIFIER)
                      .setValue(clinician.getOrganization().getResourceID()))
              .setDisplay(clinician.getOrganization().name));
      ...
      // Code
      practitionerRoleExercice.addCode(mapCodeToCodeableConcept(
                      new Code("https://mos.esante.gouv.fr/NOS/TRE_R21-Fonction/FHIR/TRE-R21-Fonction", "FON-24", "Médecin de santé publique"),
                      null));
      // Specialty
      practitionerRoleExercice.addSpecialty(mapCodeToCodeableConcept(
                      new Code("https://mos.esante.gouv.fr/NOS/TRE_R38-SpecialiteOrdinale/FHIR/TRE-R38-SpecialiteOrdinale", "SM54", "Médecine générale (SM)"),
                      null));

      // Location
      practitionerRoleExercice.addLocation()
              .setIdentifier(new Identifier()
                      .setSystem(SYNTHEA_IDENTIFIER)
                      .setValue(clinician.getOrganization().getResourceLocationID()))
              .setDisplay(clinician.getOrganization().name);
      ...
      if (clinician.getOrganization().phone != null
              && !clinician.getOrganization().phone.isEmpty()) {
        practitionerRoleExercice.addTelecom(new ContactPoint()
                .setSystem(ContactPointSystem.PHONE)
                .setValue(clinician.getOrganization().phone));
      }
      practitionerRoleExercice.addTelecom(practitionerResource.getTelecomFirstRep());
      ...
      newEntry(bundle, practitionerRoleExercice, uuid);
      ...
      practitionerRolePro.setPractitioner(new Reference()
              ...
              .setDisplay(practitionerResource.getNameFirstRep().getNameAsSingleString()));
      // Organization
      practitionerRolePro.setOrganization(new Reference()
              .setIdentifier(new Identifier()
                      .setSystem(SYNTHEA_IDENTIFIER)
                      .setValue(clinician.getOrganization().getResourceID()))
              .setDisplay(clinician.getOrganization().name));
      ...
      // Code
      practitionerRolePro.addCode(mapCodeToCodeableConcept(
              new Code("https://mos.esante.gouv.fr/NOS/TRE_R21-Fonction/FHIR/TRE-R21-Fonction", "FON-24", "Médecin de santé publique"),
              null));
      // Specialty
      practitionerRolePro.addSpecialty(mapCodeToCodeableConcept(
              new Code("https://mos.esante.gouv.fr/NOS/TRE_R38-SpecialiteOrdinale/FHIR/TRE-R38-SpecialiteOrdinale", "SM54", "Médecine générale (SM)"),
              null));

      UUID proOrigUuid = UUID.fromString(uuid);
      String proUuid = ExportHelper.buildUUID(proOrigUuid.getLeastSignificantBits(),
              proOrigUuid.getMostSignificantBits(),
              "PractitionerRole2 for Clinician " + proOrigUuid);

      newEntry(bundle, practitionerRolePro, proUuid);
    }
